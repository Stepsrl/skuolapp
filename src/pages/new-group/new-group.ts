...
export class NewGroupPage {
  ...
  ionViewDidLoad() {
    ...
    this.dataProvider.getCurrentUser().snapshotChanges().subscribe((accountRes) => {
      console.log("load:"+JSON.stringify(accountRes));
      let account = {$key: accountRes.key, ... accountRes.payload.val()};
      if (!this.groupMembers) {
        this.groupMembers = [account];
      }
      if (account.friends) {
          console.log("friends:"+JSON.stringify(account));
        for (var i = 0; i < account.friends.length; i++) {
            console.log("friend:"+JSON.stringify(account.friends[i]));
          this.dataProvider.getUser(account.friends[i].$key).snapshotChanges().subscribe((friendRes) => {

            if(friendRes.key != null){
                console.log("friend:"+JSON.stringify(friendRes));

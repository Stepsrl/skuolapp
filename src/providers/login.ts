...
export class LoginProvider {
    ...
    constructor(public loadingProvider: LoadingProvider, public alertProvider: AlertProvider, public zone: NgZone, public googleplus: GooglePlus,
        ...
        const unsubscribe =firebase.auth().onAuthStateChanged((user) => {
            //console.log("carica login provider");
            if (user) {
               // console.log(user);
                this.zone.run(() => {
                    // this.navCtrl.setRoot(Settings.homePage, { animate: false });
                    this.navCtrl.setRoot(Settings.homePage);
                    // unsubscribe();
                });
    ...
    setNavController(navCtrl) {
        console.log("carica login setNavController");
    ...
    emailLogin(email, password) {
        ...
        firebase.auth().signInWithEmailAndPassword(email, password).then((success) => {
            this.loadingProvider.hide();
            //console.log("utente login:"+JSON.stringify(firebase.auth().currentUser));
            //console.log("utente login Profilo:"+firebase.auth().currentUser.phoneNumber);


            firebase.auth().currentUser.getIdTokenResult()
                .then((idTokenResult) => {
                    // Confirm the user is an Admin.
                    if (!!(idTokenResult.claims.tipo == "D")) {
                        // Show admin UI.
                    //    console.log("utente login ADMIN:"+JSON.stringify(idTokenResult));

                        // bisogna caricare le classi dove e' abilitato il docente


                    } else {
                        // Show regular user UI.
                        console.log("utente login Profilo:"+JSON.stringify(idTokenResult));
                    }
                })
                .catch((error) => {
                    console.log(error);
            ...
            // firebase.auth().currentUser.profilo = "Docente";
            //  console.log("utente login:"+JSON.stringify(firebase.auth().currentUser));
